...
.db-mesh::before{
  ...
  background:
    radial-gradient(ellipse 60% 50% at 20% 40%,rgba(99,102,241,0.18) 0%,transparent 70%),
...
.db-nav{
  ...
  padding:0 32px;transition:background .4s,border-color .4s;
}
.db-nav.scrolled{
...
.db-logo{display:flex;align-items:center;gap:10px;margin-right:auto;cursor:pointer;text-decoration:none}
.db-logo-mark{width:34px;height:34px;position:relative;flex-shrink:0}
.db-logo-text{font-family:var(--sans);font-weight:800;font-size:1.1rem;color:#F8FAFC;letter-spacing:-0.03em;line-height:1}
.db-logo-text span{color:var(--indigo3);font-weight:400}
.db-logo-sub{font-size:.6rem;color:var(--muted);letter-spacing:.1em;text-transform:uppercase;margin-top:1px}
...
.db-badge{
  ...
  opacity:0;animation:db-up .65s .05s cubic-bezier(.16,1,.3,1) forwards;
}
.db-badge-dot{
...
.db-h1{
  ...
  opacity:0;animation:db-up .65s .14s cubic-bezier(.16,1,.3,1) forwards;
}
.db-h1 em{font-style:italic;color:var(--indigo3)}
.db-h1 .db-grd{
  font-style:italic;
...
.db-h-sub{
  ...
  opacity:0;animation:db-up .65s .22s cubic-bezier(.16,1,.3,1) forwards;
}

.db-capture-wrap{
  display:flex;flex-direction:column;align-items:center;gap:14px;width:100%;
  opacity:0;animation:db-up .65s .3s cubic-bezier(.16,1,.3,1) forwards;
}
.db-cap-btn-solo{
  background:linear-gradient(135deg,var(--indigo),var(--indigo2));
  border:none;color:#fff;
...
.db-cap-btn-solo:hover{transform:translateY(-3px);box-shadow:0 16px 48px rgba(99,102,241,0.55)}
.db-h-micro{
...
.db-trow{
  ...
  opacity:0;animation:db-up .65s .4s cubic-bezier(.16,1,.3,1) forwards;
}
.db-tp{
  display:flex;align-items:center;gap:6px;
  background:rgba(255,255,255,0.04);border:1px solid var(--border);border-radius:100px;
  padding:6px 13px;font-size:.75rem;font-weight:600;color:var(--muted);
}

/* ── Mockup ── */
.db-mock-wrap{
  max-width:660px;width:100%;margin:0 auto;position:relative;
  opacity:0;animation:db-up .85s .5s cubic-bezier(.16,1,.3,1) forwards;
}
.db-mock-wrap::before{
...
.db-mtitle{flex:1;text-align:center;font-size:.73rem;color:var(--muted);font-weight:600;letter-spacing:.03em}

/* Scan bar */
.db-scan-wrap{
  margin:0 16px 0;padding:12px;
  border:2px dashed rgba(99,102,241,0.18);border-radius:12px;
  background:rgba(99,102,241,0.04);position:relative;overflow:hidden;
}
.db-scan-line{
  position:absolute;left:0;right:0;height:2px;top:0;
  background:linear-gradient(90deg,transparent,var(--indigo3),transparent);
  animation:db-scan 2.4s ease-in-out infinite;
}
.db-scan-inner{display:flex;align-items:center;gap:10px}
.db-scan-doc{
  width:38px;height:48px;border-radius:7px;flex-shrink:0;
  background:rgba(99,102,241,0.1);border:1px solid rgba(99,102,241,0.15);
  display:flex;align-items:center;justify-content:center;font-size:1.1rem;
}
.db-scan-lines{flex:1;display:flex;flex-direction:column;gap:5px}
.db-scan-bar-item{height:6px;border-radius:3px;background:rgba(255,255,255,0.07);overflow:hidden;position:relative}
.db-scan-bar-item::after{
  content:'';position:absolute;left:-100%;top:0;bottom:0;width:100%;
  background:linear-gradient(90deg,transparent,rgba(99,102,241,0.4),transparent);
  animation:db-shimmer 2.4s ease-in-out infinite;
}
.db-scan-bar-item:nth-child(1){width:100%}.db-scan-bar-item:nth-child(1)::after{animation-delay:.1s}
.db-scan-bar-item:nth-child(2){width:78%}.db-scan-bar-item:nth-child(2)::after{animation-delay:.3s}
.db-scan-bar-item:nth-child(3){width:60%}.db-scan-bar-item:nth-child(3)::after{animation-delay:.5s}
.db-scan-badge{
  background:var(--green2);border:1px solid rgba(16,185,129,0.25);
  border-radius:7px;padding:4px 10px;font-size:.64rem;font-weight:800;color:var(--green);white-space:nowrap;
}
...
.db-mshl{font-size:.83rem;font-weight:700;color:#F1F5F9}
.db-mshr{display:flex;align-items:center;gap:5px;font-size:.69rem;color:var(--muted)}
...
.db-dico{width:32px;height:32px;border-radius:8px;display:flex;align-items:center;justify-content:center;font-size:.88rem;flex-shrink:0}
.db-dn{font-size:.8rem;font-weight:700;color:#F1F5F9}
.db-ds{font-size:.67rem;color:var(--muted);margin-top:1px}
.db-dbadge{margin-left:auto;font-size:.62rem;font-weight:800;padding:3px 8px;border-radius:5px;white-space:nowrap}
...
.db-mav{
  ...
  display:flex;align-items:center;justify-content:center;
  font-size:.54rem;font-weight:800;margin-left:-6px;
}
.db-mav:first-child{margin-left:0}

/* Floating cards — CONTAINED inside mock-wrap, no overflow */
.db-float-cards{
...
.db-fcard-head{display:flex;align-items:center;gap:8px;margin-bottom:6px}
.db-fcard-ico{
  width:26px;height:26px;border-radius:7px;
  display:flex;align-items:center;justify-content:center;font-size:.75rem;flex-shrink:0;
}
.db-fcard-title{font-size:.69rem;font-weight:800;line-height:1.3}
.db-fcard-body{font-size:.65rem;color:var(--muted);line-height:1.5;margin-bottom:7px}
.db-fcard-cta{
  font-size:.63rem;font-weight:800;padding:3px 9px;border-radius:5px;
  background:rgba(16,185,129,0.1);color:var(--green);border:none;
...
.db-sbadge{display:flex;align-items:center;gap:7px;font-size:.77rem;font-weight:700;color:var(--muted)}
...
.db-sl{font-size:.78rem;color:var(--muted);font-weight:600}

/* ══ SECTION HEADERS ══ */
.db-slabel{
  display:inline-block;font-size:.67rem;font-weight:800;
  letter-spacing:.18em;text-transform:uppercase;color:var(--indigo3);margin-bottom:12px;
}
.db-sh2{
  font-family:var(--serif);font-weight:400;
  font-size:clamp(1.9rem,3.2vw,2.8rem);
  line-height:1.12;letter-spacing:-.02em;color:#F8FAFC;margin-bottom:14px;
}
.db-sh2 em{font-style:italic;color:var(--indigo3)}
.db-sp{font-size:.92rem;line-height:1.82;color:rgba(238,242,255,0.55);max-width:460px}
...
.db-fdesc{font-size:.91rem;line-height:1.84;color:rgba(238,242,255,0.55);margin-bottom:22px}
.db-fbulls{list-style:none;display:flex;flex-direction:column;gap:9px}
.db-fbull{
  display:flex;align-items:flex-start;gap:10px;
  font-size:.85rem;color:rgba(238,242,255,0.6);line-height:1.6;
}
.db-fbull-ic{
...
.db-fvis{
  ...
  transition:transform .4s cubic-bezier(.16,1,.3,1);width:100%;
}
...
.db-fvt{font-size:.72rem;color:var(--muted);font-weight:600;margin-left:3px}
...
.db-kvk{font-size:.74rem;color:var(--muted)}
.db-kvv{font-size:.74rem;color:#F1F5F9;font-weight:700}
...
.db-acav{width:30px;height:30px;border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:.66rem;font-weight:800}
...
.db-shlt{flex:1;font-size:.71rem;color:var(--indigo3);font-weight:600;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
.db-shcopy{background:linear-gradient(135deg,var(--indigo),var(--indigo2));color:#fff;border:none;font-size:.68rem;font-weight:800;padding:5px 11px;border-radius:6px;cursor:pointer;font-family:var(--sans)}
.db-shtag{font-size:.68rem;font-weight:700;padding:3px 8px;border-radius:5px;background:rgba(255,255,255,0.04);color:var(--muted);border:1px solid rgba(255,255,255,0.06);margin-right:5px;display:inline-block;margin-bottom:3px}
...
.db-scheme-inner{max-width:1120px;margin:0 auto;display:grid;grid-template-columns:1fr 1fr;gap:64px;align-items:center}
.db-scheme-vis{
  background:var(--navy2);border:1px solid rgba(255,255,255,0.08);
  border-radius:16px;overflow:hidden;
}
...
.db-sv-body{padding:14px}
.db-sv-label{font-size:.67rem;font-weight:800;letter-spacing:.12em;text-transform:uppercase;color:var(--green);margin-bottom:12px}
.db-sv-item{
  padding:11px 13px;background:rgba(16,185,129,0.04);
  border:1px solid rgba(16,185,129,0.1);border-radius:10px;margin-bottom:8px;
}
.db-sv-name{font-size:.78rem;font-weight:700;color:#F1F5F9;margin-bottom:3px;display:flex;align-items:center;justify-content:space-between}
.db-sv-desc{font-size:.68rem;color:var(--muted);line-height:1.5;margin-bottom:7px}
.db-sv-tags{display:flex;gap:5px;flex-wrap:wrap}
.db-sv-tag{font-size:.6rem;font-weight:700;padding:3px 8px;border-radius:5px;background:var(--green2);color:var(--green);border:1px solid rgba(16,185,129,0.12)}
.db-sv-match{font-size:.64rem;font-weight:800;padding:2px 8px;border-radius:4px;background:var(--green2);color:var(--green)}
.db-sv-cta{
  margin-top:12px;width:100%;padding:9px;
  background:rgba(16,185,129,0.08);border:1px solid rgba(16,185,129,0.2);
  border-radius:9px;font-family:var(--sans);font-size:.75rem;
  font-weight:700;color:var(--green);cursor:pointer;transition:all .2s;
}
.db-sv-cta:hover{background:rgba(16,185,129,0.14)}
...
.db-htitle{font-weight:700;font-size:.91rem;color:#F1F5F9;margin-bottom:6px}
.db-hdesc{font-size:.79rem;line-height:1.72;color:var(--muted)}

/* ══ MARQUEE ══ */
.db-mqs{padding:60px 0;overflow:hidden}
.db-mqs-in{max-width:1120px;margin:0 auto;padding:0 32px}
.db-mq-lbl{text-align:center;font-size:.67rem;font-weight:800;letter-spacing:.18em;text-transform:uppercase;color:var(--muted);margin-bottom:22px}
...
.db-mpill{
  ...
  padding:7px 15px;font-size:.8rem;font-weight:600;color:rgba(238,242,255,0.55);
...
.db-rline{display:flex;align-items:center;justify-content:center;gap:10px;margin-bottom:44px;font-size:.84rem;color:rgba(238,242,255,0.55)}
.db-rstars{color:var(--amber);font-size:.92rem;letter-spacing:2px}
...
.db-tcst{color:var(--amber);font-size:.76rem;letter-spacing:3px;margin-bottom:11px}
.db-tcq{font-size:.86rem;line-height:1.78;color:rgba(238,242,255,0.55);font-style:italic;margin-bottom:18px}
.db-tcp{display:flex;align-items:center;gap:10px}
.db-tcav{width:36px;height:36px;border-radius:50%;display:flex;align-items:center;justify-content:center;font-weight:800;font-size:.86rem;flex-shrink:0}
.db-tcn{font-weight:700;font-size:.84rem;color:#F1F5F9}
.db-tcr{font-size:.7rem;color:var(--muted);margin-top:2px}
...
.db-fqqt{font-weight:700;font-size:.91rem;color:#F1F5F9}
.db-fqic{
  width:26px;height:26px;border-radius:8px;flex-shrink:0;
  background:rgba(255,255,255,0.05);border:1px solid rgba(255,255,255,0.08);
  display:flex;align-items:center;justify-content:center;
  font-size:.85rem;color:var(--muted);transition:all .22s;
}
.db-fqic.db-op{background:var(--indigo4);border-color:rgba(99,102,241,0.3);color:var(--indigo3);transform:rotate(45deg)}
.db-fqa{overflow:hidden;max-height:0;transition:max-height .35s ease,padding .35s}
.db-fqa.db-op{max-height:180px;padding-bottom:16px}
.db-fqat{font-size:.85rem;line-height:1.8;color:rgba(238,242,255,0.55)}
...
.db-ctah{
  ...
  line-height:1.1;letter-spacing:-.02em;color:#F8FAFC;
  max-width:560px;margin:0 auto 16px;position:relative;z-index:1;
}
.db-ctah em{font-style:italic;color:var(--indigo3)}
.db-ctap{font-size:.97rem;color:rgba(238,242,255,0.55);max-width:400px;margin:0 auto 32px;line-height:1.78;position:relative;z-index:1}
...
.db-ctab1{
  ...
  box-shadow:0 6px 22px rgba(99,102,241,0.4);transition:all .22s;
}
...
.db-ctab2{
  ...
  padding:12px 24px;border-radius:11px;cursor:pointer;transition:all .22s;
}
...
.db-foot-brand p{font-size:.81rem;color:var(--muted);line-height:1.72;margin-top:10px;max-width:230px}
.db-fct{font-size:.68rem;font-weight:800;letter-spacing:.14em;text-transform:uppercase;color:var(--muted);margin-bottom:14px}
.db-fl{display:block;font-size:.81rem;color:var(--muted);background:none;border:none;cursor:pointer;padding:0;text-align:left;margin-bottom:9px;transition:color .15s;font-family:var(--sans)}
...
.db-fcopy{font-size:.74rem;color:rgba(255,255,255,0.22)}
...
@keyframes db-scan{0%{top:0;opacity:0}10%{opacity:1}90%{opacity:1}100%{top:100%;opacity:0}}
@keyframes db-shimmer{0%{left:-100%}100%{left:200%}}

.db-rv{opacity:0;transform:translateY(26px);transition:opacity .55s ease,transform .55s ease}
.db-rv.db-in{opacity:1;transform:translateY(0)}
.db-d1{transition-delay:.06s}.db-d2{transition-delay:.12s}.db-d3{transition-delay:.18s}.db-d4{transition-delay:.24s}
...
@media(max-width:900px){
  ...
  .db-nbtn{padding:7px 14px;font-size:.82rem}
  .db-nbtn2{margin-left:0;padding:8px 16px;font-size:.82rem}
  .db-hero{padding:120px 24px 72px;min-height:auto}
  .db-h1{font-size:clamp(2.2rem,6vw,3.4rem)}
  .db-h-sub{font-size:.97rem;max-width:100%}
  ...
  .db-scheme-sec{padding:64px 24px}
  .db-scheme-inner{grid-template-columns:1fr}
...
@media(max-width:600px){
  ...
  .db-nbtn{font-size:.78rem;padding:6px 12px;border-radius:8px}
  .db-nbtn2{font-size:.78rem;padding:7px 12px;margin-left:0}
  .db-hero{padding:100px 16px 56px}
  .db-h1{font-size:clamp(1.9rem,8vw,2.6rem);line-height:1.1}
  .db-h-sub{font-size:.92rem;margin-bottom:28px}
  .db-badge{font-size:.66rem;padding:4px 12px 4px 7px}
  .db-cap-btn-solo{font-size:.93rem;padding:15px 28px;width:100%;max-width:340px}
  .db-h-micro{font-size:.72rem;gap:10px}
  .db-trow{display:grid;grid-template-columns:1fr 1fr;gap:6px;margin-top:28px;margin-bottom:36px}
  .db-tp{font-size:.71rem;padding:6px 10px;justify-content:flex-start;border-radius:10px}
  .db-mock-wrap{max-width:100%}
  .db-mock{animation:none;border-radius:14px}
  .db-float-cards{flex-direction:column;align-items:center}
  .db-fcard{max-width:100%;min-width:0;width:100%}
...
const PILLS_RAW = ['🪪 Aadhaar Card','💳 PAN Card','🛂 Passport','🚗 Driving Licence','🗳️ Voter ID','📄 Income Certificate','🏠 Domicile Certificate','👶 Birth Certificate','💍 Marriage Certificate','🎓 Degree Certificate','🏥 Medical Reports','🏘️ Property Docs','🛡️ Insurance Papers','💵 ITR Documents','📋 Caste Certificate','🏫 School LC'];
...
const TESTI = [
  ...
  {av:'P',name:'Priya Mehta',   role:'Business Owner, Mumbai',           c:'#10B981',bg:'rgba(16,185,129,0.1)', q:'"The 90-day expiry alert arrived for my passport. I renewed on time. Without DocBox I would have completely missed it."'},
  {av:'A',name:'Arjun Singh',   role:'Software Engineer, Bengaluru',     c:'#34D399',bg:'rgba(52,211,153,0.1)', q:'"DocBox found a scholarship I had no idea existed. My sister got ₹8,000. The scheme discovery feature alone is worth everything."'},
];

const FAQS = [
  {q:'Is my Aadhaar data safe on DocBox?',a:'Yes. AES-256 encryption at rest, TLS 1.3 in transit. We never read or share your documents. DocBox is fully DPDP Act compliant with India-region servers.'},
  {q:'What document types does DocBox recognise?',a:'20+ types including Aadhaar, PAN, Passport, Driving Licence, Voter ID, Birth/Marriage Certificates, Insurance Papers, ITR, Degree Certificates, Property Docs, Medical Reports, Caste/Income/Domicile Certificates and more.'},
  {q:'Can family members see all my documents?',a:'No. Every document has individual permission settings — View, Download, or Share. Nobody sees anything you haven\'t explicitly allowed. You can change permissions instantly.'},
  {q:'What if I lose my phone?',a:'Nothing is stored on your device. Sign in from any browser and your vault is exactly where you left it. Recently accessed documents are cached for offline viewing.'},
  {q:'What is Government Scheme Discovery?',a:'DocBox analyses your documents nightly and matches you to eligible schemes on MyScheme.gov.in, scholarships.gov.in, and Maharashtra state portals. You get a notification when a new scheme matches your family.'},
  {q:'Is DocBox free?',a:'Yes — there is a free plan with 5 GB family storage. No credit card required to get started.'},
];
      ...
      <div className="db-page">
        ...
        <nav className={`db-nav${scrolled ? ' scrolled' : ''}`}>
          <div className="db-nav-row">
            ...
            <div className="db-nlinks">
              {[['features','Features'],['how-it-works','How it works'],['schemes','Govt Schemes'],['testimonials','Reviews'],['faq','FAQ']].map(([id, label]) => (
        ...
        <section className="db-hero">
          ...
          <div className="db-cx db-hero-inner">
            ...
            <h1 className="db-h1">
              Find any document<br/><span className="db-grd">before panic sets in.</span>
            </h1>

            <p className="db-h-sub">
              One secure vault for every Aadhaar, PAN, passport and certificate your family owns. Reads blurry WhatsApp photos. Alerts you 90 days before expiry. Discovers government schemes you qualify for.
            </p>
            ...
            {/* Mockup — fully contained, no absolute overflow */}
            <div className="db-mock-wrap">
              <div className="db-mock">
                ...
                <div className="db-scan-wrap">
                  ...
                  <div className="db-scan-inner">
                    <div className="db-scan-doc">🪪</div>
                    <div className="db-scan-lines">
                      <div className="db-scan-bar-item"/>
                      <div className="db-scan-bar-item"/>
                      <div className="db-scan-bar-item"/>
                    </div>
                    <div className="db-scan-badge">Aadhaar ✓</div>
                  </div>
                </div>
                  ...
                  <div className="db-mbody">
                    ...
                    <div className="db-mfoot">
                      <div style={{display:'flex',alignItems:'center',gap:7}}>
                        ...
                        <span style={{fontSize:'.69rem',color:'var(--muted)'}}>4 members</span>
                      </div>
                      <span style={{fontSize:'.69rem',color:'var(--muted)'}}>↑ Drop to upload</span>
                    </div>
                  </div>
                </div>
              </div>

              {/* Floating info cards — BELOW the mockup, inside the wrapper */}
              <div className="db-float-cards">
                {/* Expiry alert card */}
                <div className="db-fcard">
                  <div className="db-fcard-head">
                    <div className="db-fcard-ico" style={{background:'rgba(245,158,11,0.12)',border:'1px solid rgba(245,158,11,0.2)'}}>⏰</div>
                    <div className="db-fcard-title" style={{color:'#F59E0B'}}>Passport expiring in 47 days</div>
                  </div>
                  <div className="db-fcard-body">Priya's passport — renew before it's too late. Renewal takes 4–6 weeks.</div>
                  <button className="db-fcard-cta" style={{background:'rgba(245,158,11,0.1)',color:'#F59E0B'}}>Set reminder →</button>
                </div>
                {/* Scheme discovery card */}
                <div className="db-fcard">
                  <div className="db-fcard-head">
                    <div className="db-fcard-ico" style={{background:'rgba(16,185,129,0.12)',border:'1px solid rgba(16,185,129,0.2)'}}>🎯</div>
                    <div className="db-fcard-title" style={{color:'var(--green)'}}>New scheme found</div>
                  </div>
                  <div className="db-fcard-body">PM Ayushman Bharat — ₹5L health cover. Your family may be eligible.</div>
                  <button className="db-fcard-cta" onClick={() => navigate('/signup')}>Check eligibility →</button>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="db-sstrip db-rv">
          <div className="db-sstrip-in">
            {[['🔒','256-bit AES encryption'],['🛡️','Zero-knowledge architecture'],['🇮🇳','India-region servers'],['📜','DPDP Act compliant'],['🕵️','Immutable audit log']].map(([ic,lb]) => (
              <div key={lb} className="db-sbadge"><span style={{fontSize:'.92rem'}}>{ic}</span>{lb}</div>
            ))}
          </div>
        </div>
        ...
        <section className="db-stats">
          <div className="db-stats-grid">
            {[['20+','Document types recognised','db-d1'],['2,400+','Families using DocBox','db-d2'],['5 GB','Free family storage','db-d3'],['90 days','Advance expiry alerts','db-d4']].map(([n,l,d]) => (
          ...
          <div className="db-cx">
                ...
                <div className="db-fvis">
                  ...
                  <div className="db-fvb">
                    <div style={{border:'2px dashed rgba(99,102,241,0.18)',borderRadius:10,padding:'16px 12px',textAlign:'center',marginBottom:12,background:'rgba(99,102,241,0.04)'}}>
                      ...
                      <div style={{fontSize:'.78rem',color:'rgba(238,242,255,0.7)',fontWeight:700,marginBottom:3}}>Drop your document here</div>
                      <div style={{fontSize:'.69rem',color:'var(--muted)'}}>Blurry WhatsApp photos work too</div>
                    </div>
                    ...
                    <div style={{marginTop:10,padding:'8px 12px',background:'rgba(16,185,129,0.08)',border:'1px solid rgba(16,185,129,0.2)',borderRadius:8,fontSize:'.74rem',color:'#10B981',fontWeight:700}}>
            ...
            <div className="db-frow db-flip db-rv">
              ...
              <div>
                <div className="db-fvis">
                  ...
                  <div className="db-fvb">
                    {[
                      {icon:'🛂',name:"Priya's Passport",days:47,  pct:15,c:'#EF4444',gs:'rgba(239,68,68,0.3)',  badge:'Urgent'},
                      {icon:'🚗',name:'Driving Licence',  days:120, pct:45,c:'#F59E0B',gs:'rgba(245,158,11,0.3)',badge:'Soon'},
                      {icon:'🛡️',name:'Health Insurance', days:290, pct:80,c:'#10B981',gs:'rgba(16,185,129,0.3)',badge:'OK'},
                      {icon:'🪪',name:'Aadhaar Card',     days:'Permanent',pct:100,c:'#6366F1',gs:'rgba(99,102,241,0.3)',badge:'✓'},
                    ].map(d => (
                      <div key={d.name} className="db-er">
                        <div className="db-erl">
                          <span style={{fontSize:'.86rem'}}>{d.icon}</span>
                          <div>
                            <div style={{fontSize:'.78rem',fontWeight:700,color:'#F1F5F9'}}>{d.name}</div>
                            <div style={{fontSize:'.67rem',color:'var(--muted)'}}>{typeof d.days==='number'?`${d.days} days left`:d.days}</div>
                          </div>
                        </div>
                        <div className="db-erbar-w"><div className="db-erbar" style={{width:`${d.pct}%`,background:d.c,boxShadow:`0 0 6px ${d.gs}`}}/></div>
                        <span style={{fontSize:'.62rem',fontWeight:800,padding:'3px 8px',borderRadius:5,background:`${d.c}22`,color:d.c,whiteSpace:'nowrap'}}>{d.badge}</span>
                      </div>
                    ))}
                  </div>
                </div>
            ...
            <div className="db-frow db-rv">
              <div className="db-ftxt">
                ...
                <p className="db-fdesc">View, Download, or Share — set permissions per document. Each family member sees exactly what you've allowed, nothing more. Emergency access requests are approved by you.</p>
                <ul className="db-fbulls">
                    ...
                    {[
                      ...
                      <div key={r.n} className="db-acrow">
                        <div className="db-acp">
                          ...
                          <div>
                            <div style={{fontSize:'.78rem',fontWeight:700,color:'#F1F5F9'}}>{r.n}</div>
                            <div style={{fontSize:'.67rem',color:'var(--muted)'}}>{r.rel}</div>
                          </div>
                        </div>
                        <span style={{fontSize:'.67rem',fontWeight:800,padding:'3px 9px',borderRadius:100,background:r.pc,color:r.pcl}}>{r.perm}</span>
                      </div>
                    ))}
                  </div>
                </div>
                ...
                <div className="db-fvis">
                  ...
                  <div className="db-fvb">
                    <div className="db-shbox">
                      <div style={{fontSize:'.74rem',color:'var(--muted)',fontWeight:600,marginBottom:8}}>Share link generated ✓</div>
                    ...
                    <div style={{display:'flex',gap:6}}>
                      {['WhatsApp','Email','Copy link'].map(a => (
                        <div key={a} style={{flex:1,padding:'7px',background:'rgba(99,102,241,0.06)',border:'1px solid rgba(99,102,241,0.14)',borderRadius:8,fontSize:'.72rem',fontWeight:700,color:'var(--indigo3)',textAlign:'center',cursor:'pointer'}}>{a}</div>
                      ))}
                    </div>
                  </div>
                </div>
        ...
        {/* ══ GOVT SCHEME SECTION ══ */}
        <section className="db-scheme-sec" id="schemes">
          <div className="db-cx">
            <div className="db-scheme-inner">
              <div className="db-rv">
                <div className="db-slabel">Unique feature</div>
                <h2 className="db-sh2">Discover schemes<br/><em>you didn't know existed.</em></h2>
                <p className="db-sp">DocBox scans MyScheme.gov.in, scholarship portals, and state welfare sites every night. When your documents match a scheme, you get notified — automatically, for free.</p>
                <ul className="db-fbulls" style={{marginTop:20}}>
                  {['Scans MyScheme.gov.in nightly','Maharashtra state schemes included','Scholarships.gov.in for students','Zero effort — fully automatic'].map(b => (
                    <li key={b} className="db-fbull"><span className="db-fbull-ic">✓</span>{b}</li>
                  ))}
                </ul>
              </div>
              <div className="db-rv">
                <div className="db-scheme-vis">
                  <div className="db-sv-header">
                    <div className="db-fvd" style={{background:'#FF5F57'}}/><div className="db-fvd" style={{background:'#FEBC2E'}}/><div className="db-fvd" style={{background:'#28C840'}}/>
                    <span className="db-fvt">Schemes matched to your family</span>
                  </div>
                  <div className="db-sv-body">
                    <div className="db-sv-label">🎯 2 new matches found tonight</div>
                    {[
                      {name:'PM Ayushman Bharat',desc:'₹5 lakh annual health cover for eligible families',tags:['Health','Central Govt'],match:'96%'},
                      {name:'Maharashtra Scholarship',desc:'Annual scholarship for college students — ₹8,000/yr',tags:['Education','State'],match:'88%'},
                    ].map(s => (
                      <div key={s.name} className="db-sv-item">
                        <div className="db-sv-name">
                          {s.name}
                          <span className="db-sv-match">{s.match} match</span>
                        </div>
                        <div className="db-sv-desc">{s.desc}</div>
                        <div className="db-sv-tags">
                          {s.tags.map(t => <span key={t} className="db-sv-tag">{t}</span>)}
                        </div>
                      </div>
                    ))}
                    <button className="db-sv-cta" onClick={() => navigate('/signup')}>
                      Check all eligible schemes →
                    </button>
                  </div>
                </div>
              </div>
        ...
        <section className="db-ctas">
          <div className="db-ctabox db-rv">
            <h2 className="db-ctah">Stop losing documents<br/><em>your family actually needs.</em></h2>
            <p className="db-ctap">Join 2,400+ Indian families who found everything in 3 seconds — free forever plan, no credit card.</p>
        ...
        <footer className="db-foot">
          <div className="db-foot-grid">
            ...
            <div>
              ...
              {['Features','How it works','Govt Schemes','Security','Changelog'].map(l => (
