...
import {
  Menu as MenuIcon, Notifications, CloudDone,
...
const TopBar = ({ onMenuClick, drawerWidth, isMobile }) => {
  ...
  const handleNotificationClick = () => {
    ...
    // Reload count after short delay (page will mark items read)
  ...
  return (
    <AppBar position="fixed" elevation={0}
      sx={{
        ...
        borderBottom: '1px solid #E2E8F0', boxShadow: '0 1px 0 #E2E8F0',
        color: '#0F172A', zIndex: (theme) => theme.zIndex.drawer - 1,
      }}>
      <Toolbar sx={{ minHeight: { xs: 56, sm: 64 }, px: { xs: 2, sm: 3 }, gap: 1 }}>
        ...
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5 }}>
          ...
          <Tooltip title="Offline Mode Active" arrow>
            <Box sx={{
              ...
              background: 'rgba(16,185,129,0.08)', border: '1px solid rgba(16,185,129,0.2)', cursor: 'default',
            }}>
              <CloudDone sx={{ fontSize: 14, color: '#10B981' }} />
              <Typography sx={{ fontSize: '0.75rem', fontWeight: 500, color: '#10B981' }}>Online</Typography>
            </Box>
          </Tooltip>
          ...
          <Box onClick={handleMenu}
            sx={{
              ...
              pl: 1, pr: 1.5, py: 0.75, ml: 0.5, borderRadius: '99px',
              border: '1px solid #E2E8F0', background: '#FFFFFF', cursor: 'pointer',
              transition: 'all 150ms ease',
              '&:hover': { background: '#F8F9FC', borderColor: '#CBD5E1', boxShadow: '0 2px 8px rgba(15,23,42,0.06)' },
            }}>
            ...
            <Typography sx={{
              display: { xs: 'none', sm: 'block' }, fontSize: '0.875rem', fontWeight: 600,
              color: '#0F172A', maxWidth: 120, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap',
            }}>
        ...
        <Menu anchorEl={anchorEl} open={Boolean(anchorEl)} onClose={handleClose}
          ...
          PaperProps={{ elevation: 3, sx: {
            mt: 1, minWidth: 200, borderRadius: '12px', border: '1px solid #E2E8F0', overflow: 'visible',
            '&::before': {
              content: '""', display: 'block', position: 'absolute',
              top: -5, right: 16, width: 10, height: 10, background: '#fff',
              borderTop: '1px solid #E2E8F0', borderLeft: '1px solid #E2E8F0',
              transform: 'rotate(45deg)', zIndex: 0,
            },
          }}}>
          <Box sx={{ px: 2, pt: 1.5, pb: 1 }}>
            <Typography sx={{ fontWeight: 600, fontSize: '0.875rem', color: '#0F172A' }}>{user?.fullName}</Typography>
          ...
          <MenuItem onClick={() => { navigate('/settings'); handleClose(); }}
            ...
            <ListItemIcon sx={{ minWidth: 'unset' }}><Settings sx={{ fontSize: 16, color: '#475569' }} /></ListItemIcon>
            Settings
          </MenuItem>
          <Divider sx={{ my: 0.5 }} />
          <MenuItem onClick={handleLogout}
            sx={{ gap: 1.5, px: 2, py: 1, fontSize: '0.875rem', color: '#EF4444', borderRadius: '6px', mx: 0.5, mb: 0.5 }}>
            <ListItemIcon sx={{ minWidth: 'unset' }}><ExitToApp sx={{ fontSize: 16, color: '#EF4444' }} /></ListItemIcon>
            Log out
          </MenuItem>
        </Menu>
      </Toolbar>
